...
from datasets.Oxford.oxford_raw import OxfordPointCloudLoader
...
class TrainingDataset(Dataset):
    def __init__(self, dataset_path: str, dataset_type: str, pickle_name: str,seqence:str, transform=None, set_transform=None,n_points=4096):
        ...
        if dataset_type == 'kitti':
            self.query_filepath = os.path.join(dataset_path,seqence, pickle_name)
        ...
        self.pc_loader = get_pointcloud_loader(self.dataset_type,self.n_points)
...
class EvaluationSet:
    ...
    def get_map_positions(self):#读取每个map的position(x,y)
...
def get_pointcloud_loader(dataset_type,n_points=4096) -> PointCloudLoader:
    if dataset_type == 'mulran':
        return MulranPointCloudLoader()
    elif dataset_type == 'southbay':
        return SouthbayPointCloudLoader()
    elif dataset_type == 'kitti':
        return KittiPointCloudLoader(n_points)
    elif dataset_type == 'oxford':
        return OxfordPointCloudLoader(n_points)
    else:
        raise NotImplementedError(f"Unsupported dataset type: {dataset_type}")

def get_all_sequence(query_filepath,test_sequence,pickle_name) -> Dict[int, TrainingTuple]:
    global sequence
    sequence.remove(test_sequence)
    d:Dict[int, TrainingTuple]= {}
    for s in sequence:
        pickle_path = os.path.join(query_filepath, s,pickle_name)
        temp:Dict[int, TrainingTuple] = pickle.load(open(pickle_path, 'rb'))
        length = len(d)
        for i in temp:
            temp[i].id += length
            temp[i].positives += length
            temp[i].non_negatives += length
            d[len(d)] = temp[i]
