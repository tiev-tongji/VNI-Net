...
from datasets.kitti.kitti_train import KittiTraining6DOFDataset
...
def make_datasets(params: TrainingParams, sequence:str,local=False, validation=True):
    # Create training and validation datasets
    datasets = {}
    # train_transform = TrainTransform(params.aug_mode)# 训练点云数据增强
    ...
    # datasets['global_train'] = TrainingDataset(params.dataset_folder, params.dataset, params.pickle_name,params.test_sequence,'train',
    #                                            transform=None, set_transform=None)
    datasets = TrainingDataset(params.dataset_folder, params.dataset, params.pickle_name,sequence,
                                               transform=None, set_transform=None)
    # if validation:
    #     datasets['global_val'] = TrainingDataset(params.dataset_folder, params.dataset, params.pickle_name,params.test_sequence,'test')
    ...
    #                                                   set_transform=train_set_transform,n_points=params.model_params.n_points)

    # if local:
    #     if "mulran" in params.dataset:
    ...
    #                                                           params.model_params.quantizer,
    #                                                           rot_max=params.rot_max, trans_max=params.trans_max)
    #     elif "kitti" in params.dataset:
    #         datasets['local_train'] = KittiTraining6DOFDataset(params.dataset_folder, params.train_file,
    #                                                         params.model_params.local_transform,
    #                                                         rot_max=params.rot_max, trans_max=params.trans_max)
    #         if validation:
    #             datasets['local_val'] = KittiTraining6DOFDataset(params.dataset_folder, params.val_file,
    #                                                           params.model_params.local_transform,
    #                                                           rot_max=params.rot_max, trans_max=params.trans_max)
    return datasets
...
def make_collate_fn(dataset: TrainingDataset):
    # quantizer: converts to polar (when polar coords are used) and quantizes
    def collate_fn(data_list):
        # Constructs a batch object
        clouds = [e[0] for e in data_list]#点云坐标
        labels = [e[1] for e in data_list]#点云id
...
def make_collate_fn_6DOF(dataset: TrainingDataset, device):

    # ego_converted: function to convert from cartesian to polar coordinates; if None no conversion is done
    # set_transform: the transform to be applied to all batch elements
    def collate_fn(data_list):
        # Constructs a batch object
        anchor_clouds = [e[0] for e in data_list] #anchor点云
        positive_clouds = [e[1] for e in data_list]#positive点云 这里的positive是随机选择一个
        rel_transforms = [e[2] for e in data_list]#原始位姿×随机位姿
        ...
        xyz_batch1 = torch.cat(xyz_batch1, 0).float()#把所有点拼到一起
        ...
        return {"anc_pcd": xyz_batch1, "pos_pcd": xyz_batch2,
                "T_gt": trans_batch, "len_batch": len_batch}

    return collate_fn


def make_dataloaders(params: TrainingParams,sequence:str, debug=False, device='cpu', local=False, validation=True):
    """
    ...
    datasets = make_datasets(params,sequence,local=local, validation=validation)

    # dataloders = {}
    train_sampler = BatchSampler(datasets, batch_size=params.batch_size)


    # Collate function collates items into a batch and applies a 'set transform' on the entire batch
    train_collate_fn = make_collate_fn(datasets)
    dataloders = DataLoader(datasets, batch_sampler=train_sampler,
                                            collate_fn=train_collate_fn, num_workers=params.num_workers,
                                            pin_memory=True)
    # if validation and 'global_val' in datasets:
    #     val_collate_fn = make_collate_fn(datasets['global_val'])
    ...
    # if local:
        # train_collate_fn_loc = make_collate_fn_6DOF(datasets['local_train'], device)
        # val_collate_fn_loc = make_collate_fn_6DOF(datasets['local_val'], device)
...
def in_sort_array_negtive(e:int,label:int,dataset:TrainingDataset) -> bool:
    if dataset.queries[e].rel_scan_filepath[0:2] != dataset.queries[label].rel_scan_filepath[0:2]:
        return True
    else:
        array = dataset.queries[label].non_negatives
        pos = np.searchsorted(array, e)
        if pos == len(array) or pos == -1:
            return False
        else:
