...
class KittiPointCloudLoader(PointCloudLoader):
    ...
    def read_pc(self, file_pathname: str):
        ...
        # Returns Nx3 tensor
        pc = []
        # point_dir = 'vel_random_incloud_sample_4096'
        point_dir = 'vel_random_sample_4096'
        file_pathname=os.path.join(file_pathname[0:file_pathname.find('velodyne')],point_dir,file_pathname[file_pathname.rfind('/')+1:])
        
        if point_dir == 'vel_random_incloud_sample_4096':
        ...
        # PC in Mulran and Kitti is of size [num_points, 4] -> x,y,z,reflectance
...
class KittiSequence(Dataset):
    ...
    def __init__(self, dataset_root: str, sequence_name: str, split: str,
        ...
        #                      timestamps difference)/media/autolab/disk_3T/222nas/Datasets/KITTI/odometry/sequences/00/
...
class KittiSequences(Dataset):
    """
    Multiple Kitti sequences indexed as a single dataset. Each element is identified by a unique global index.
    """
    def __init__(self, dataset_root: str, sequence_names: List[str], split: str):
        ...
        for seq_name in self.sequence_names:
            ds = KittiSequence(self.dataset_root, seq_name, split=split)
            sequences.append(ds)
